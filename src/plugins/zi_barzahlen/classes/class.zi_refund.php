...
class zi_refund {
  ...
  protected function _checkRefundAmount($transaction_id, $amount) {
    ...
    if(round($result->fields['amount'] - $result->fields['zi_refund'] - $amount, 2) < 0) {
      barzahlen_log::error('class/refund: requested refund amount too high',
                               array($transaction_id, $amount));
      return false;
    }

    if($amount <= 0) {
      barzahlen_log::error('class/refund: requested refund amount equal or smaller 0',
                              array($transaction_id, $amount));
      return false;
    }

    if(!is_numeric($amount)) {
      barzahlen_log::error('class/refund: requested refund amount is no numeric value',
                               array($transaction_id, $amount));
      return false;
    }
  ...
  protected function _registerRefundId($transaction_id, $amount) {
    ...
    if($refund->getOriginTransactionId() != $transaction_id) {
      barzahlen_log::error('class/refund: origin transaction id not equal transaction id',
                              array($transaction_id, $amount, $refund->getXmlArray()));
      return false;
    }
